...
exports.CREATE_ORDER = {
  ...
  async resolve(parent, args, context) {
    ...
    let resAdditionalCost = await conn
      .promise()
      .query(sqlAddtionalCost, [args.region.toLowerCase()]);
    let extraCharge = 0;
    ...
    const result = await conn
      ...
      .query(sql, [
        ...
        args.region.toLowerCase(),
        args.city.toLowerCase(),
        args.street.toLowerCase(),
        args.streetNumber.toLowerCase(),
        args.serviceId,
        orderSubTotal,
        orderTotal,
        Date.now(),
    ...
    if (args.details.length > 0) {
      for (let el of args.details.slice()) {
        let sqlODets = `INSERT INTO order_details (order_id, partner_id, appointment_start, appointment_end, appointment_order, created, updated)
                        VALUES(?,?,?,?,?,?,?) `;
        let resQDets = await conn
          .promise()
          .query(sqlODets, [
            lastInsId,
            el.partner_id,
            el.appointment_start,
            el.appointment_end,
            el.appointment_order,
            Date.now(),
            Date.now(),
          ]);
      ...
      if (parseInt(resSPrice[0][0].appointments_number) > 1) {
        for (
          let i = 2;
          i <= parseInt(resSPrice[0][0].appointments_number);
          i++
        ) {
...
exports.DELETE_ORDER = {
  ...
  async resolve(parent, args, context) {
    ...
    await conn
      .promise()
      .query(`DELETE FROM order_details WHERE order_id = ?`, [args.id]);
