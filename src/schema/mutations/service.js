...
exports.CREATE_SERVICE = {
  ...
  async resolve(parent, args, context) {
    ...
    const { service_name, type, appointments_number, appointment_duration, service_cost } = args;      
    const sql = `INSERT INTO services
    (service_name, m_type_id, appointments_number, appointment_duration, service_cost, created, updated) VALUES (?,?,?,?,?,?,?)`;
    const result = await conn.promise().query(sql, [service_name, type, appointments_number, appointment_duration, service_cost, Date.now(), Date.now()]);
...
exports.UPDATE_SERVICE = {
  ...
  async resolve(parent, args, context) {
    ...
    await conn.promise().query(`UPDATE services SET service_name = ? , m_type_id = ? , appointments_number = ? , appointment_duration = ? , service_cost = ? , updated = ? WHERE s_id = ? `, [args.service_name, args.type, args.appointments_number, args.appointment_duration, args.service_cost, Date.now(), args.id]);
...
exports.DELETE_SERVICE = {
  ...
  async resolve(parent, args, context) {
    ...
    await conn.promise().query(`DELETE FROM services WHERE s_id = ?`, [args.id]);
