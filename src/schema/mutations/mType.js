...
exports.CREATE_M_TYPE = {
  ...
  async resolve(parent, args, context) {
    ...
    const result = await conn.promise().query(sql, [name, Date.now(), Date.now()]);
...
exports.UPDATE_M_TYPE = {
  ...
  async resolve(parent, args, context) {
    ...
    await conn.promise().query(`UPDATE m_types SET mt_name = ? , updated = ? WHERE mt_id = ? `, [args.name, Date.now(), args.id]);
